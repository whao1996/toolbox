<template>
  <div class="page-container">
    ...
    <el-card class="info-card" shadow="never">
      ...
      >
        <template #default>
          上传 PDF 文件后，系统将自动提取文本内容并生成 Word（.docx）文档。转换完成后将自动下载。
          <br />
          <strong>注意：</strong>复杂排版（如多栏、特殊字体等）可能无法完全保留格式。
        </template>
      </el-alert>
    </el-card>
      ...
      <div class="status-content">
        <div v-if="loading" class="status-loading">
          ...
          <div class="status-text">
            ...
            <p class="status-sub">PDF 文件较大时转换可能需要较长时间</p>
          </div>
        </div>
        ...
        <div v-else-if="status === 'success'" class="status-result status-success">
          ...
          <div class="status-text">
            ...
            <p class="status-sub">Word 文件已开始下载，请检查浏览器下载列表</p>
          </div>
        </div>
    ...
    <el-card class="guide-card" shadow="never">
      ...
      <el-steps :active="stepActive" align-center finish-status="success">
        <el-step title="上传 PDF" description="点击或拖拽 PDF 文件到上传区域" />
        <el-step title="开始转换" description="点击【开始转换并下载】按钮" />
        <el-step title="下载 Word" description="转换完成后自动下载 .docx 文件" />
      </el-steps>
    </el-card>
  </div>
</template>
...
async function handleConvert() {
  ...
  try {
    ...
    const fileName = selectedFile.value.name.toLowerCase().replace('.pdf', '.docx')
